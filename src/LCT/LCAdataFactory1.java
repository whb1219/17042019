...
//            ParameterSetToolBox.setParameters(parameterSet, Arrays.asList(parameter));
    ...
    private static void setPropertiesOfEngineCI(CatalogueItem engineCI)
    {
        ...
        KeyValue enginePriceKV = KeyValueHelper.createKeyValueOfType(engineCI, "pricePropKV", "price", ValueType.T_OBJECT,costItem);
    ...
    private static void setPropertiesOfBatteryCI(CatalogueItem batteryCI)
    {
        ...
        KeyValue batteryPriceKV = KeyValueHelper.createKeyValueOfType(batteryCI, "pricePropKV", "price", ValueType.T_OBJECT, costItem);
...
//                batteryRh3KV, batteryRh4KV, batteryRh5KV, batteryRh6KV, batteryRh7KV, batteryTypeKV));
    ...
    public static Set<CatalogueItem> getCatalogueItemsByClassName(String catalogueItemClassName)
    {
        ...
        Catalogue cat = cm.open(catPOIDs.stream().filter(catPoid -> catPoid.getName().equals(catalogueName)).findFirst().orElse(null));
...
//        } 
        ...
        if (result.getParameters() == null) {
            ...
            result.getParameters().add(lifeCycleTotalCostsKV);
        }
    }   
...
CostClassification costClassification = projOM.createInformationObject(CostClassification.class,
        ...
        KeyValue RPNCostKV = KeyValueHelper.createKeyValueOfType(result, "EPCostKV", "RPNCost",
                ValueType.T_OBJECT, costItem);
